...
use warehouse_receipts::receipt::{Self, VaultAuth};
...
fun mint_items_to_depositor(
    ...
    storage_unit.game_item_to_chain_inventory_test<Character>(
        &character, &owner_cap,
        item_id, type_id, volume, quantity,
        ts.ctx(),
    );
    character.return_owner_cap(owner_cap, receipt);
    ts::return_shared(character);
...
fun partial_redeem_via_split() {
    ...
    mint_items_to_depositor(&mut ts, depositor_id, storage_id,
        LENS_ITEM_ID, LENS_TYPE_ID, LENS_VOLUME, LENS_QUANTITY);
...
fun multiple_independent_vaults() {
    ...
    let nwn_2 = network_node::anchor(
        ...
        NWN_ITEM_ID + 10, NWN_TYPE_ID, LOCATION_HASH,
    ...
    let storage_id_2 = {
        ...
        let su = storage_unit::anchor(
            &mut registry, &mut nwn_2, &character, &admin_acl,
            STORAGE_ITEM_ID + 10, STORAGE_TYPE_ID, MAX_CAPACITY, LOCATION_HASH, ts.ctx(),
        );
        let id = object::id(&su);
    ...
    mint_items_to_depositor(&mut ts, depositor_id, storage_id_1,
        LENS_ITEM_ID, LENS_TYPE_ID, LENS_VOLUME, LENS_QUANTITY);
    mint_items_to_depositor(&mut ts, depositor_id, storage_id_2,
        AMMO_ITEM_ID, AMMO_TYPE_ID, AMMO_VOLUME, AMMO_QUANTITY);
    ...
    ts::next_tx(&mut ts, depositor());
    {
        ...
        let deposit_receipt = receipt::deposit_for_receipt(
            &mut su1, &depositor_char, &owner_cap,
            &vault_config_1, collection_1,
            LENS_TYPE_ID, LENS_QUANTITY, ts.ctx(),
        );
        transfer::public_transfer(deposit_receipt, depositor());
...
fun deposit_with_wrong_vault_config_aborts() {
    ...
    let nwn_2 = network_node::anchor(
        &mut registry, &character, &admin_acl,
        NWN_ITEM_ID + 20, NWN_TYPE_ID, LOCATION_HASH,
        FUEL_MAX_CAPACITY, FUEL_BURN_RATE_IN_MS, MAX_PRODUCTION, ts.ctx(),
    );
    let nwn_id_2 = object::id(&nwn_2);
    nwn_2.share_network_node(&admin_acl, ts.ctx());
    ...
    let storage_id_2 = {
        ...
        let su = storage_unit::anchor(
            &mut registry, &mut nwn_2, &character, &admin_acl,
            STORAGE_ITEM_ID + 20, STORAGE_TYPE_ID, MAX_CAPACITY, LOCATION_HASH, ts.ctx(),
        );
        let id = object::id(&su);
        su.share_storage_unit(&admin_acl, ts.ctx());
        ts::return_shared(admin_acl);
...
fun non_owner_cannot_initialize_vault() {
    ...
    let nwn_2 = network_node::anchor(
        &mut registry, &character, &admin_acl,
        NWN_ITEM_ID + 40, NWN_TYPE_ID, LOCATION_HASH,
        FUEL_MAX_CAPACITY, FUEL_BURN_RATE_IN_MS, MAX_PRODUCTION, ts.ctx(),
    );
    let nwn_id_2 = object::id(&nwn_2);
    nwn_2.share_network_node(&admin_acl, ts.ctx());
    ...
    let _storage_id_2 = {
        ...
        let su = storage_unit::anchor(
            &mut registry, &mut nwn_2, &character, &admin_acl,
            STORAGE_ITEM_ID + 40, STORAGE_TYPE_ID, MAX_CAPACITY, LOCATION_HASH, ts.ctx(),
        );
        let id = object::id(&su);
        su.share_storage_unit(&admin_acl, ts.ctx());
        ts::return_shared(admin_acl);
...
fun redeem_with_mismatched_collection_aborts() {
    ...
    let nwn_2 = network_node::anchor(
        &mut registry, &character, &admin_acl,
        NWN_ITEM_ID + 30, NWN_TYPE_ID, LOCATION_HASH,
        FUEL_MAX_CAPACITY, FUEL_BURN_RATE_IN_MS, MAX_PRODUCTION, ts.ctx(),
    );
    let nwn_id_2 = object::id(&nwn_2);
    nwn_2.share_network_node(&admin_acl, ts.ctx());
    ...
    let storage_id_2 = {
        ...
        let su = storage_unit::anchor(
            &mut registry, &mut nwn_2, &character, &admin_acl,
            STORAGE_ITEM_ID + 30, STORAGE_TYPE_ID, MAX_CAPACITY, LOCATION_HASH, ts.ctx(),
        );
        let id = object::id(&su);
        su.share_storage_unit(&admin_acl, ts.ctx());
        ts::return_shared(admin_acl);
    ...
    receipt::redeem_receipt(
        deposit_receipt, &mut su1, &depositor_char,
        &vault_config_2, &mut collection, ts.ctx(),
    );
