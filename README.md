...
Before _warehouse receipts_, every behavior a Storage Unit needed - item trading, lending, escrow, guild hangars, etc - had to be built into a single monolithic extension contract. Adding a new use case meant rewriting and redeploying the extension, and the SSU owner had to trust that one contract to handle everything.
